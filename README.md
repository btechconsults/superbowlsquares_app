...
```bash
superbowlsquares_app$ sam build --use-container
```
...
```bash
superbowlsquares_app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
superbowlsquares_app$ sam local start-api
superbowlsquares_app$ curl http://localhost:3000/
```
...
```bash
superbowlsquares_app$ sam logs -n HelloWorldFunction --stack-name "superbowlsquares_app" --tail
```
...
```bash
superbowlsquares_app$ pip install -r tests/requirements.txt --user
# unit test
superbowlsquares_app$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
superbowlsquares_app$ AWS_SAM_STACK_NAME="superbowlsquares_app" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "superbowlsquares_app"
```
